        ...
        var DEBUGGING_DATA = false;
        var DEBUGGING_MAXOBJECTS = false;
        var DEBUGGING_SCENE = false;
    ...
    */
    ...
    // INITIALISATION FUNCTIONS
        function init_data_array() {
            ...
            for (i=0 ; i< NFSR ; i++) {
                ...
                for (_=0 ; _<NFSRPARS ; _++) {
                    data[DATAIDX_fsrs][i][DATAIDX_FSR_PAR_id].push(PAR_NOID_DEFAULT);
                }
        ...
        function init_maxObj_array() {
            ...
            for (i=0 ; i < NFSR; i++) {
                ...
                for (var _=0 ; _ < NFSRNOTEOUTS; _++) {
                    MAXOBJECTS[MAXOBJECT_FSRS_IDX][i][MAXOBJECT_FSR_NOTEOUT_IDX].push("placeholder_noteOut");   //[MAXOBJECT_FSRS_IDX][fsrIdx][MAXOBJECT_FSR_PAR_IDX][parIdx][MAXOBJECT_FSR_PAR_ID_IDX]           
                }
            } 
        ...
        function get_maxObj_names() {
            ...
            for (var i=0 ; i<NFLEX; i++) { 
                ...
                var object = root.getnamed(objectName);
                if (typeof(object) != "undefined") {
                    //post("MAXOBJECT_FLEX_MUTE_MAIN_IDX, idx ",i, " OBJECT ",object, " , ",objectName, "\n")
                    MAXOBJECTS[MAXOBJECT_FLEXOS_IDX][i][MAXOBJECT_FLEX_MUTE_MAIN_IDX] = object;
                    //post("try recalling object: ", MAXOBJECTS[MAXOBJECT_FLEXOS_IDX][i][MAXOBJECT_FLEX_MUTE_MAIN_IDX] , "\n")
                ...
                if (typeof(object) != "undefined") {
                    //post("MAXOBJECT_FLEX_MUTE_IDX, idx ",i, " OBJECT ",object, " , ",objectName, "\n")
                    MAXOBJECTS[MAXOBJECT_FLEXOS_IDX][i][MAXOBJECT_FLEX_MUTE_IDX] = object;
                    //post("try recalling object: ", MAXOBJECTS[MAXOBJECT_FLEXOS_IDX][i][MAXOBJECT_FLEX_MUTE_IDX] , "\n")
                } else if (DEBUGGING_MAXOBJECTS) {
                    DEBUGGING_ARRAY.push(objectName);
                    found = false;
                }
            ...
            for (i=0 ; i < NFSR; i++) {
                ...
                object = root.getnamed(objectName);
                if (typeof(object) != "undefined") {
                    MAXOBJECTS[MAXOBJECT_FSRS_IDX][i][MAXOBJECT_FSR_MUTE_MAIN_IDX] = object;
                } else if (DEBUGGING_MAXOBJECTS) {
                    found = false;
                }
                //MAXOBJECTS[MAXOBJECT_FSRS_IDX][i][MAXOBJECT_FSR_MUTE_MAIN_IDX] = setup.getnamed("fsr_mute_main_"+FSR_FINGERS[i].toString());
                ...
                for (j=0 ; j<NFSRPARS ; j++) {
                    object = setup.getnamed(varname_constructor_fsrNoteOut(FSR_FINGERS[i],j)); 
                    if (typeof(object) != "undefined") {
                        MAXOBJECTS[MAXOBJECT_FSRS_IDX][i][MAXOBJECT_FSR_NOTEOUT_IDX][j] = object;
                    } else if (DEBUGGING_MAXOBJECTS) {
                        found = false;
                    }
                }
    ...
    // FLEX ENV FUNCTIONS
        ...
        function refresh_flex_env_menu(dataIdx,preset) {
            if (DEBUGGING_SCENE) {
                post("refreshing env menu for flex data idx ", dataIdx, "\n");
            }
            ...
            if (DEBUGGING_SCENE) {
                post("setting env menu for flex data idx ", dataIdx, " to ", preset,  "\n");
            }
        ...
        function call_flex_env(dataIdx,preset) {
            ...
            if (!(dictEnvs.contains(preset))) {
                outlet(OUTLET_PRINT,"ERROR : CANNOT FIND ENV " + preset, ". Switching to Default env.\n");
                preset = "Default 1:1";
    ...
    // CALL/SAVE SCENE FUNCTIONS
        function loadScene() {
            ...
            if (dictPresets.contains(scene)) {
                ...
                // FOR EACH FSR
                for (var i = 0; i < NFSR; i++) {
                    // GET FLEX MUTE
                    var mute =  dictPresets.get(key_constructor_fsrMute(scene,i));
                    refresh_fsr_mute(i,mute);   

                    // GET FSR GATE
                    var gate =  dictPresets.get(key_constructor_fsrGate(scene,i));
                    refresh_fsr_gate(i,gate);

                    // GET PARAMETERS 
                    for (var j = 0; j < NFSRPARS; j++) {
                        var id = dictPresets.get(key_constructor_fsrPar(scene,i,j));
                        if (id == "id") {
                            id = 0;
                        } 
                        data[DATAIDX_fsrs][i][DATAIDX_FSR_PAR_id][j] = id;
                        refresh_fsr_param(i,j,id);
                    }

                    // GET NOTEOUTS 
                    for (var j = 0; j < NFSRNOTEOUTS; j++) {
                        var noteOut = dictPresets.get(key_constructor_fsrNoteOut(scene,i,j));
                        
                        data[DATAIDX_fsrs][i][DATAIDX_FSR_noteouts][j] = noteOut;
                        refresh_fsr_noteOut(i,j,noteOut);
                    }
                }
                
                writeSceneToHistory(scene)
                if (!(initialised)) { // init scene call
    ...
    // SEND DATA TO MAX OBJECTS FUNCTIONS
        ...
        function refresh_flex_mute(flexIdx,val) { 
            //post("function refresh_flex_mute. idx: ",flexIdx, ", val: ",val,"\n")
            MAXOBJECTS[MAXOBJECT_FLEXOS_IDX][flexIdx][MAXOBJECT_FLEX_MUTE_MAIN_IDX].message(val);
        ...
        function refresh_fsr_mute(fsrIdx,val) {
            MAXOBJECTS[MAXOBJECT_FSRS_IDX][fsrIdx][MAXOBJECT_FSR_MUTE_MAIN_IDX].message(val);
            MAXOBJECTS[MAXOBJECT_FSRS_IDX][fsrIdx][MAXOBJECT_FSR_MUTE_IDX].message(val);
        }

        function refresh_fsr_gate(fsrIdx,val) {
            MAXOBJECTS[MAXOBJECT_FSRS_IDX][fsrIdx][MAXOBJECT_FSR_GATE_IDX].message(val);
        }

        function refresh_fsr_param(fsrIdx,parIdx,val) { 
            MAXOBJECTS[MAXOBJECT_FSRS_IDX][fsrIdx][MAXOBJECT_FSR_PAR_IDX][parIdx].message(val);   
        }

        function refresh_fsr_noteOut(fsrIdx,parIdx,val) { 
            MAXOBJECTS[MAXOBJECT_FSRS_IDX][fsrIdx][MAXOBJECT_FSR_NOTEOUT_IDX][parIdx].message(val);   
        }
        ...
        function write_scene_to_dict(scene) { 
            ...
            for (var i = 0; i < NFSR; i++) {
                // SAVE FLEX MUTE
                if (DEBUGGING_SCENE) {
                    post(key_constructor_fsrMute(scene,i), " ", data[DATAIDX_fsrs][i][DATAIDX_FSR_mute]); //data[CLOCK_DEFAULT,[ [0] ] ]
                } 
                dictPresets.replace(key_constructor_fsrMute(scene,i),data[DATAIDX_fsrs][i][DATAIDX_FSR_mute]);

                // SAVE FSR GATE
                if (DEBUGGING_SCENE) {
                    post(key_constructor_fsrGate(scene,i), " ", data[DATAIDX_fsrs][i][DATAIDX_FSR_gate]); //data[CLOCK_DEFAULT,[ [0] ] ]
                } 
                dictPresets.replace(key_constructor_fsrGate(scene,i),data[DATAIDX_fsrs][i][DATAIDX_FSR_gate]);

                // GET PARAMETERS 
                for (var j = 0; j < NFLEXPARS; j++) {
                    if (DEBUGGING_SCENE) {
                        post(key_constructor_fsrPar(scene,i,j), data[DATAIDX_fsrs][i][DATAIDX_FSR_PAR_id][j], "\n")
                        
                    }
                    dictPresets.replace(key_constructor_fsrPar(scene,i,j) , data[DATAIDX_fsrs][i][DATAIDX_FSR_PAR_id][j]);
                }

                // GET NOTEOUTS
                for (var j = 0; j < NFSRNOTEOUTS; j++) {
                    if (DEBUGGING_SCENE) {
                        post(key_constructor_fsrNoteOut(scene,i,j), data[DATAIDX_fsrs][i][DATAIDX_FSR_noteouts][j], "\n")
                        
                    }
                    dictPresets.replace(key_constructor_fsrNoteOut(scene,i,j), data[DATAIDX_fsrs][i][DATAIDX_FSR_noteouts][j]);
                }
            }
        }
    ...
    // CONSTRUCTORS
        ...
        function key_constructor_fsrMute(scene,frsIdx) {
            return scene + keyFsr_n + frsIdx.toString() + keyMute;
        }

        function key_constructor_fsrGate(scene,frsIdx) {
            return scene + keyFsr_n + frsIdx.toString() + keyFsrGate;
        }

        function key_constructor_fsrPar(scene,frsIdx,parIdx) { 
            return scene + keyFsr_n + frsIdx.toString() + keyPar + parIdx.toString();
        }
        
        function key_constructor_fsrNoteOut(scene,frsIdx,parIdx) { 
            return scene + keyFsr_n + frsIdx.toString() + keyFsrNote + parIdx.toString();
        }
    ...
    //REAL-TIME DATA PROCESSING FUNCTIONS
        ...
        function muteFsr() { 
            if (initialised) {
                ...
                var dataIdx = FSR_FINGERS.indexOf(fsrIdx); 
                if (dataIdx != -1) {
                    data[DATAIDX_fsrs][dataIdx][DATAIDX_FSR_mute]  = arguments[1]; 
                    if (DEBUGGING_DATA) {
                        post("fsr data idx ", dataIdx, ", mute: ",
                            data[DATAIDX_fsrs][dataIdx][DATAIDX_FSR_mute], "\n")
                    }
                }
            }
        }  
        ...
        function fsrGates() {
            if (initialised) {
                for (var i = 0; i < arguments.length; i++) {
                    var dataIdx = FLEX_FINGERS.indexOf(i); 
                    if (dataIdx != -1) {
                        data[DATAIDX_fsrs][dataIdx][DATAIDX_FSR_gate] = arguments[i];
                        if (DEBUGGING_DATA) {
                            post("fsr data idx ", dataIdx, ", gate: ",
                                data[DATAIDX_fsrs][dataIdx][DATAIDX_FSR_gate], "\n")
                        }
                    }
                }
            }
        ...
        function idParFsr() { 
            if (initialised) {
                ...
                if (dataIdx != -1) {
                    data[DATAIDX_fsrs][dataIdx][DATAIDX_FSR_PAR_id][parIdx] = id;
                    
                    if (DEBUGGING_DATA) {
                        post("fsr data idx ", dataIdx, ", par ", parIdx, ", id: ",
                            data[DATAIDX_fsrs][dataIdx][DATAIDX_FSR_PAR_id][parIdx], "\n")
                    }
                }
            }
        }
            ...
            if (initialised) {
                ...
                if (dataIdx != -1) {
                    for (var i = 1; i < arguments.length; i++) {
                        data[DATAIDX_fsrs][dataIdx][DATAIDX_FSR_noteouts][i-1] = arguments[i];
                        if (DEBUGGING_DATA) {
                            post("fsr data idx ", dataIdx, ", noteout ", i-1, ", midi: ",
                                data[DATAIDX_fsrs][dataIdx][DATAIDX_FSR_noteouts][i-1], "\n")
                        }
                    }
                }
            }
