...
const int CHECK_CONNECTION_TIME_MS = 2500;
unsigned long REFRESH_MS  = 50;
...
unsigned long checkConnection_lastMillis = 0;
...
//BATTERY
const float BATTERY_VOLTAGE_LOOKUP[] = {4.07 , 3.83 , 3.67 , 3.56, 3.5 , 3.467 , 3.445 , 3.43 , 3.39 , 3.36, 3.0}; 
...
void sendBackOnline() {
    OSCMessage msgBackOnlineOut("/back-online");
    Udp.beginPacket(laptop_IP, sendPort);  // Send to laptop
    msgBackOnlineOut.send(Udp);
    Udp.endPacket();
    msgBackOnlineOut.empty();
}
...
void checkConnection() {
    if (WiFi.status() != WL_CONNECTED) {
        ...
        if (lastUpdateTime - checkConnection_lastMillis > CHECK_CONNECTION_TIME_MS) {
            WiFi.disconnect();
            WiFi.reconnect();
            checkConnection_lastMillis = millis();    
        } 
    }
